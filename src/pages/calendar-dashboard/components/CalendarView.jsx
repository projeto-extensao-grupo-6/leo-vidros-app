      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="w-full max-w-sm lg:max-w-md bg-white rounded-xl shadow-2xl overflow-hidden border border-gray-100"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-4 lg:p-6">
              <div className="flex items-center gap-3 lg:gap-4 mb-3 lg:mb-4">
                <div className="p-2 lg:p-3 bg-red-100 rounded-full shrink-0">
      ...
      <AnimatePresence>
        {details && (
          <motion.div
            ...
            onClick={onClose}
          >
            <div className="flex items-start justify-center min-h-screen pt-10 lg:pt-20 pb-6 lg:pb-10 px-2 lg:px-4">
              <motion.div
                initial={{ scale: 0.95, opacity: 0, y: 20 }}
                animate={{ scale: 1, opacity: 1, y: 0 }}
                exit={{ scale: 0.95, opacity: 0, y: 20 }}
                transition={{ duration: 0.3, ease: "easeOut" }}
                className="bg-white rounded-xl lg:rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] lg:max-h-[80vh] overflow-hidden border border-gray-200 flex flex-col"
                onClick={(e) => e.stopPropagation()}
              >
                <EventHeader
                  title={details.title}
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      <div className="grid grid-cols-7 bg-white border-b border-gray-200 shrink-0">
        {weekDaysNames.map((w) => (
          <div
            key={w}
            className="py-1.5 lg:py-2 text-center text-[10px] lg:text-xs font-bold text-gray-400 uppercase tracking-wide lg:tracking-widest"
          >
            {w}
          </div>
        ))}
      ...
      <div className="grid grid-cols-7 flex-1 auto-rows-fr gap-px bg-gray-200 overflow-hidden">
        {days.map((day) => {
          ...
          return (
            <div
              key={dateKey}
              className={`bg-white relative flex flex-col p-1 lg:p-1.5 group transition-colors hover:bg-gray-50 border-gray-200 overflow-hidden cursor-pointer ${
                !isCurrent ? "opacity-60 bg-gray-50/50" : ""
              }`}
              onClick={() => onDateClick?.(day)}
            >
              <div className="flex justify-between items-start mb-0.5 lg:mb-1 shrink-0">
                <div className="relative">
                  <span
                    className={`text-[10px] lg:text-xs font-semibold w-5 h-5 lg:w-6 lg:h-6 flex items-center justify-center rounded-lg ${isCurrentToday ? "bg-blue-600 text-white shadow-md" : "text-gray-700"}`}
                  >
                    {format(day, "d")}
                  </span>
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="hidden lg:opacity-0 lg:group-hover:opacity-100 lg:flex"
                  onClick={(e) => {
                    e.stopPropagation();
                    onDateClick?.(day);
                  }}
              ...
              <div className="flex-1 flex flex-col gap-0.5 lg:gap-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
                {dayEvents.slice(0, 4).map((evt, index) => (
                  <motion.div
                    ...
                    className="text-[8px] lg:text-[10px] px-1 lg:px-1.5 py-0.5 rounded truncate cursor-pointer hover:opacity-90 transition border-l-2 shadow-sm"
                    style={{
                  ...
                  >
                    <span className="font-bold mr-0.5 lg:mr-1 opacity-75">
                      {evt.startTime}
                    </span>
                ...
                {dayEvents.length > 4 && (
                  <div className="text-[7px] lg:text-[9px] text-gray-500 font-bold text-center pt-0.5">
...
const CalendarView = ({
  ...
  return (
    ...
    >
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between px-2 lg:px-6 py-2 lg:py-5 border-b border-gray-100 bg-white shrink-0 gap-2 lg:gap-0">
        <div className="flex items-center gap-2 lg:gap-6 w-full lg:w-auto">
          <div className="flex items-center gap-1 lg:gap-2 bg-gray-50 rounded-lg p-0.5 lg:p-1 border border-gray-100">
            <button
              onClick={handlePrev}
              className="p-1 lg:p-1.5 cursor-pointer hover:bg-white hover:shadow-sm rounded-md text-gray-600 transition"
            >
          ...
          <div className="flex items-center gap-2 lg:gap-4 flex-1 lg:flex-initial">
            <h2 className="text-base lg:text-2xl font-bold text-gray-900 capitalize tracking-tight">
              {renderHeaderTitle()}
            </h2>
            <button
              onClick={handleTodayClick}
              className="px-2 lg:px-4 py-1 lg:py-1.5 text-xs lg:text-sm font-semibold text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-100 rounded-full transition-colors whitespace-nowrap"
            >
              Hoje
            </button>
          </div>
        </div>
        <div className="flex items-center justify-between lg:justify-end gap-2 lg:gap-7 bg-white p-1 w-full lg:w-auto">
          <Button
          ...
          <div className="hidden lg:block w-px h-6 bg-gray-300 mx-2 sm:mx-4"></div>

          <div className="flex items-center gap-0 bg-white p-0.5 lg:p-1 rounded-lg border border-gray-300 shadow-sm">
            {["day", "week", "month"].map((type) => (
              <Button
                ...
                size="sm"
                startIcon={<CalendarIcon size={14} className="lg:w-[18px] lg:h-[18px]" />}
                className="text-xs lg:text-sm px-1.5 lg:px-3 py-1 lg:py-1.5"
              >
                <span className="hidden sm:inline">{{ day: "Dia", week: "Semana", month: "Mês" }[type]}</span>
                <span className="sm:hidden">{{ day: "D", week: "S", month: "M" }[type]}</span>
              </Button>
            ))}
          </div>
        </div>
