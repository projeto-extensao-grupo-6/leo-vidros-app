...
const TeamAvailability = () => {
  ...
  },
  {
    ...
    avatarAlt: "Professional headshot of blonde woman in business attire smiling",
    status: "available",
  ...
  const busyCount = teamMembers?.filter((member) => member?.status === 'busy')?.length;
  const awayCount = teamMembers?.filter((member) => member?.status === 'away')?.length;
      ...
      <div className="grid grid-cols-3 gap-2 mb-4 p-3 bg-muted rounded-modern">
        <div className="text-center">
          <div className="flex items-center justify-center space-x-1 mb-1">
            ...
            <span className="text-xs font-medium text-text-primary">{availableCount}</span>
          </div>
        ...
        <div className="text-center">
          <div className="flex items-center justify-center space-x-1 mb-1">
            ...
            <span className="text-xs font-medium text-text-primary">{busyCount}</span>
          </div>
        ...
        <div className="text-center">
          <div className="flex items-center justify-center space-x-1 mb-1">
            ...
            <span className="text-xs font-medium text-text-primary">{awayCount}</span>
          </div>
        ...
        <div
          ...
          className="flex items-start space-x-3 p-2 hover:bg-muted rounded-modern transition-micro group">
            ...
            <div className="relative flex-shrink-0">
              ...
              <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 ${getStatusColor(member?.status)} rounded-full border-2 border-card`}></div>
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              member?.status === 'busy' ? 'bg-error/10 text-error' : 'bg-warning/10 text-warning'}`
              }>
