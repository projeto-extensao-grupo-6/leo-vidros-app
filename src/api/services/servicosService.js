...
class ServicosService extends BaseService {
    ...
    async buscarPorId(id) {
        ...
        if (result.success && !result.data?.servico) {
            return { success: false, data: null, error: 'Este pedido não é um serviço', status: 400 };
        }
        return result;
    }
    ...
    async criarServico(servicoData) {
        if (!servicoData.servico) {
            return { success: false, data: null, error: 'Dados do serviço são obrigatórios', status: 400 };
        }
